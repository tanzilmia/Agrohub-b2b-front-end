...
const PopularBlogs = () => {
  ...
  useEffect(() => {
    try {
      axios
        .get(`https://agrohub-b2b-new-server.vercel.app/seller/letest-blogs`)
        .then((res) => {
  ...
  return (
    <>
      <h1 className="text-center text-4xl my-5 font-semibold dark:text-gray-300">
        Popular Blog
      </h1>
        ...
        {popularBlog.length &&
          popularBlog?.map((blog) => (
            <div className="w-full sm:w-1/2 md:w-1/3 p-4" key={blog._id}>
              <div className="h-full bg-white dark:bg-[#4B5563] shadow-md rounded-md overflow-hidden">
                <img
