...
const PaymentSuccess = () => {
    ...
    useEffect(() => {
        fetch(`https://agrohub-b2b-backend.vercel.app/payment-gateway/orders-by-transaction-id/${transactionId}`)
            .then(res => res.json())
        ...
        <div className='max-h-screen'>
            <div className="flex justify-center items-center">
                <div className="w-full flex flex-col justify-center items-center p-6 m-auto">
                    <div ref={ref} className="mt-6 md:grid md:grid-cols-1 gap-6 p-10 text-xl font-semibold bg-white rounded-md shadow-xl shadow-rose-600/40 ring-2 ring-indigo-600">
                    ...
                    <Pdf targetRef={ref} filename="payment-invoice.pdf">
                        {({ toPdf }) => <button onClick={toPdf} className='border-2 px-4 py-3 mt-4'>Download</button>}
                    </Pdf>
                </div>
            </div>
        </div>
