...
const UserReview = () => {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post("https://agrohub-b2b-new-server.vercel.app/review/product_review", productrReview)
      .then((res) => {
        if (res.data) {
    ...
    try {
      if (reviews && reviews.length > 0) {
        const response = await axios.post(
          `https://agrohub-b2b-new-server.vercel.app/review/product_review/${reviews[0]._id}/review`,
          { rating: ratingNumber }
        );
        if (response.data) {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `https://agrohub-b2b-new-server.vercel.app/review/multiple_review/${products?._id}`
        );
