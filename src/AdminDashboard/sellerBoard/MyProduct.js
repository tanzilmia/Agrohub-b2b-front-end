...
const MyProduct = () => {
  ...
  useEffect(() => {
    axios
      .get(`https://agrohub-b2b-new-server.vercel.app/common/my-product?email=${user?.email}`)
      .then((res) => {
  ...
  const deleteProduct = (id) => {
    ...
    if (confirmed) {
      try {
        axios
          .delete(`https://agrohub-b2b-new-server.vercel.app/seller/delete-product?id=${id}`)
          .then((res) => {
      ...
      {Products.length ? (
        <section className="container px-4 mx-auto pt-16">
          <h1 className="text-center text-2xl dark:text-white">
            {" "}
            {Products[0]?.sellerName} Products
          </h1>
