...
const CommonDashPage = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      const productdData = await axios.get(
        `https://agrohub-b2b-new-server.vercel.app/common/my-product?email=${user?.email}`
      );
  ...
  useEffect(() => {
    axios
      .get(
        `https://agrohub-b2b-new-server.vercel.app/payment-gateway/total-mysells?email=${user?.email}`
      )
      .then((res) => {
    ...
    <div className="flex flex-col">
      ...
      <div className="flex flex-col lg:grid lg:grid-flow-col lg:grid-cols-5">
        <div className="lg:col-span-3 flex items-center mx-3 mt-5 justify-center flex-wrap dark:text-gray-300 ">
          <div className="w-full lg:w-1/2 py-6  px-6 shadow-lg hover:shadow-xl mb-3  transition duration-300 dark:bg-indigo-700 rounded ">
          ...
          <div className="w-full lg:w-1/2 py-6  px-6 shadow-lg hover:shadow-xl  mb-3  transition duration-300 dark:bg-indigo-700 rounded ml">
          ...
          <div className="w-full lg:w-1/2 py-6  px-6 shadow-lg hover:shadow-xl transition duration-300 dark:bg-indigo-700 rounded">
            <div className="font-bold text-5xl text-center text-[#6c976c] flex justify-center">
              {" "}
              <FaUsers />{" "}
            </div>
